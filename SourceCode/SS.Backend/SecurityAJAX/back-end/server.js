...
app.get('/myEndpoint', (request, response) => {
    // Logic to handle the GET request for your endpoint
    response.send('This is my custom endpoint!');
});
...
app.post('/login', (request, response) => {
    const user = authenticateUser(request.body.username, request.body.password);
...
function verifyToken(request, response, next){
    ...
    if(!token){
        return response.status(403).json({ error: 'Token is required' });
    }
...
app.get('/protected', verifyToken, (response, request) => {
    response.json({message: 'Protected resource accessed successfully'});
});
*/

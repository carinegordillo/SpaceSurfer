...
// /* 
// can only modify start time end time, not the date 
// */

// nEEd to check if: reservation is active, reservation is not passed, reservation is not cancelled
// after we validate that, we  allow the modification to continue by validating the new vlaues 
